/**
 * @brief  Search in Rotated Array: Given a sorted array of n integers that has been rotated an
 * unknown number of times, write code to find an element in the array. You may assume that the
 * array was originally sorted in increasing order. EXAMPLE Input Find 5 in array {15, 16, 19, 20,
 * 25, 1, 3, 4, 5, 7, 10, 14} Output 8 (the index of 5 in the array)
 *
 * @file search_rotated.h
 * @author Tushar Chugh
 */
...
int search_rotated( const std::vector<int>& input, int elem, int first, int last ) {
    ...
    if ( input[mid] > input[first] ) {
        ...
        if ( input[mid] != input[last] )
            ...
            if ( result == -1 ) return search_rotated( input, elem, mid + 1, last );
            return result;
        }
    }
}
