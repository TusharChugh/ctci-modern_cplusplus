...
TEST( PERMS_WITHOUT_DUPS_BOTTOMS_UP, EMPTY_INPUT ) {
    auto output = algorithm::perms_no_dups_bootomsup( "" );
    ASSERT_EQ( "", output.front() );
    ASSERT_EQ( 1, output.size() );
}

TEST( PERMS_WITHOUT_DUPS_BOTTOMS_UP, ONE_LETTERS ) {
    std::unordered_set<std::string> expected_output = {"a"};
    auto output                                     = algorithm::perms_no_dups_bootomsup( "a" );
    std::unordered_set<std::string> output_set;
    for ( const auto& word : output ) {
        output_set.insert( word );
    }
...
TEST( PERMS_WITHOUT_DUPS_BOTTOMS_UP, TWO_LETTERS ) {
    std::unordered_set<std::string> expected_output = {"ab", "ba"};
    auto output                                     = algorithm::perms_no_dups_bootomsup( "ab" );
    std::unordered_set<std::string> output_set;
    for ( const auto& word : output ) {
        output_set.insert( word );
    }
...
TEST( PERMS_WITHOUT_DUPS_BOTTOMS_UP, THREE_LETTERS ) {
    std::unordered_set<std::string> expected_output = {"abc", "acb", "bac", "bca", "cab", "cba"};
    auto output                                     = algorithm::perms_no_dups_bootomsup( "abc" );
    std::unordered_set<std::string> output_set;
    for ( const auto& word : output ) {
        output_set.insert( word );
    }
