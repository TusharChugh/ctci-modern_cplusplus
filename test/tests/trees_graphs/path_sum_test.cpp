...
TEST( PATH_SUM, ZERO_RUNNING ) {
    algorithm::bst<int> tree = {10, 5, 13, 3, 2, 11, 3, -1, 1};
    ASSERT_EQ( 0, algorithm::path_with_sum_running_sum( tree, 40 ) );
}

TEST( PATH_SUM, ZERO_VALUE_RUNNING ) {
    algorithm::bst<int> tree = {10, 5, 13, 3, 2, 11, 3, 1};
    ASSERT_EQ( 0, algorithm::path_with_sum_running_sum( tree, 0 ) );
}

TEST( PATH_SUM, ONE_RUNNING ) {
    algorithm::bst<int> tree = {10, 5, 13, 3, 2, 11, 3, -1, 1};
    ASSERT_EQ( 1, algorithm::path_with_sum_running_sum( tree, 8 ) );
}

TEST( PATH_SUM, TWO_RUNNING ) {
    algorithm::bst<int> tree = {10, 5, 11, 3, 2, 1, 3};
    ASSERT_EQ( 2, algorithm::path_with_sum_running_sum( tree, 21 ) );
}

TEST( PATH_SUM, THREE_RUNNING ) {
    algorithm::bst<int> tree = {10, 5, 11, 3, 2, 1, 3, 0};
    ASSERT_EQ( 3, algorithm::path_with_sum_running_sum( tree, 21 ) );
}